...
)
{
	...
	half ft = lerp
	(
		i_texture.SampleLevel((0.74 * foamUVBubbles0 + parallaxOffset) / (4.0 * cascadeData0._texelWidth), 3.0).r,
		i_texture.SampleLevel((0.74 * foamUVBubbles1 + parallaxOffset) / (4.0 * cascadeData1._texelWidth), 3.0).r,
		lodVal
	);
...
)
{
	...
	worldXZ.displaced0   -= i_texture.ScaledFloatingOriginOffset(cascadeData0);
	worldXZ.displaced1   -= i_texture.ScaledFloatingOriginOffset(cascadeData1);
	worldXZ.undisplaced0 -= i_texture.ScaledFloatingOriginOffset(cascadeData0);
	worldXZ.undisplaced1 -= i_texture.ScaledFloatingOriginOffset(cascadeData1);
